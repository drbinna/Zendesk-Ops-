...
@app.function(
    image=vllm_image,
    gpu=f"{GPU}:{N_GPU}",
    # 0 = no GPU pinned; warmed on app-open by the frontend's /api/warm ping so the
    # first answer is fast (see MIN_CONTAINERS above). 300s window keeps it alive
    # through the connect→scan→chat flow so the pre-warm isn't wasted.
